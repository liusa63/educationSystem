var mainPlatform = {
	...
	bindEvent: function(){
        ...
        $(document).on('click', '.pf-modify-pwd', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'backend/updateAdminPwd.jsp')
        });

        $(document).on('click', '.pf-notice-item', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'adminLogin.jsp')
        });
        $(document).on('click', '.pf1-notice-item', function() {
            $('#pf-page').find('iframe').eq(0).attr('src', 'login.jsp')
        });

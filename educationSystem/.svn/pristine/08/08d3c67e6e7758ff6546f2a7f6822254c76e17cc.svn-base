...
<script type="text/javascript">
	function jumpfun() {
		...
		var gopage = $("#jump").val();
		//总页数
		var countPage = ${tcPage.countPage};
		if (gopage != null && gopage != "") {
			var reg = /^\d+$/;
			if (reg.test(gopage)) {
				if (gopage > 0 && gopage <= countPage) {
					window.location.href = "${pageContext.request.contextPath}/teacherAction_pageCourse?page.currentPage="
							+ gopage;
				} else {
				...
				<div class="content">
					...
					<div class="fresh-datatables">
						<table id="datatables"
							...
							<tbody>
								<c:forEach items="${tcPage.list}" var="t">
									<tr>
										<td>${t.coName }</td>
										<td>${t.coScore}</td>
										<td>${t.roName}</td>
										<td>${t.coWeek}</td>
                                        <td>${t.enDay}</td>
                                        <td>${t.enStart}~${t.enEnd}</td>
										<td>${t.coStartTime}~${t.coEndTime}</td>
						...
						<div class="col-sm-7">
							<div class="dataTables_paginate paging_full_numbers"
								...
								<ul class="pagination">
								<li>
								第${tcPage.currentPage}页 
								共${tcPage.totalNum }条
							          共${tcPage.countPage}页
							    </li>
									<li class="paginate_button first" id="datatables_first"><a
										href="${pageContext.request.contextPath}/teacherAction_pageCourse?page.currentPage=1">首页</a>

									</li>
									<li class="paginate_button previous" id="datatables_previous">
										<c:if test="${tcPage.currentPage>1}">
											<a
												href="${pageContext.request.contextPath}/teacherAction_pageCourse?page.currentPage=${tcPage.currentPage-1}">上一页</a>
										</c:if>
									</li>

									<li class="paginate_button next" id="datatables_next"><c:if
											test="${tcPage.currentPage<tcPage.countPage}">
											<a
												href="${pageContext.request.contextPath}/teacherAction_pageCourse?page.currentPage=${tcPage.currentPage+1}">下一页</a>
										</c:if></li>
									<li class="paginate_button last" id="datatables_last"><a
										href="${pageContext.request.contextPath}/teacherAction_pageCourse?page.currentPage=${tcPage.countPage}">尾页</a>
									</li>
